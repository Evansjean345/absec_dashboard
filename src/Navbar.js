...
function Navbar() {
  ...
  const [pragmatisme, setPragmatisme] = useState('');
  const [vision, setVision] = useState('');
  const [comments, setComments] = useState('');
  const [iconBook, setIconBook] = useState('');
  const [iconBag, setIconBag] = useState('');
  const [open, setOpen] = useState(false);

  useEffect(() => {
    axios
      .get("https://caauri-api.cyclic.cloud/home/absec/6567ca9d6ef4e5557750b8d9")
      .then((res) => {
           setTitle(res.data.title)
           setTitleTwo(res.data.titleTwo)
           setSynopsis(res.data.synopsis)
           setMission(res.data.mission)
           setExcellence(res.data.excellence)
           setInnovation(res.data.innovation)
           setOuverture(res.data.ouverture)
           setPragmatisme(res.data.pragmatisme)
           setVision(res.data.vision)
           setComments(res.data.comments)
           setIconBook(res.data.iconBook)
           setIconBag(res.data.iconBag)
      })
  ...
  const handleSubmit = (e) => {
    ...
    axios
      .put(
        "https://caauri-api.cyclic.cloud/home/absec/6567ca9d6ef4e5557750b8d9",
        data
      )
  ...
  return (
    <div className="flex flex-no-wrap">
      ...
      <div className="container mx-auto py-10 h-64 md:w-4/5 w-11/12 px-6">
        ...
        <div className="w-full flex flex-wrap mb-12 rounded shadow-xl">
          <form className="flex flex-col w-full" onSubmit={handleSubmit}>
            <div className="flex">
              <div className="w-1/2 py-8 px-8">
                ...
                <Textarea
                  variant="standard"
                  label="titre page d'acceuil"
                  value={titleTwo}
                  onChange={(e) => setTitleTwo(e.target.value)}
                />
                <Textarea
                  variant="standard"
                  label="titre page d'acceuil 2"
                  value={title}
                  onChange={(e) => setTitle(e.target.value)}
                />
                <strong>Synopsis premiere Section</strong>
                <Textarea
                  variant="standard"
                  label="Premier paragraphe de l'acceuil"
                  value={synopsis}
              ...
              <div className="w-1/2 py-8 px-8">
                ...
                />
                 <strong>Block avec icon Book</strong>
                <Textarea
                  variant="standard"
                  label="book"
                  value={iconBook}
                  onChange={(e) => {
                    setIconBook(e.target.value);
                  }}
                />
                  <strong>Block avec icon Bag</strong>
                <Textarea
                  variant="standard"
                  label="bag"
                  value={iconBag}
                  onChange={(e) => {
                    setIconBag(e.target.value);
                  }}
                />
              </div>
            </div>
